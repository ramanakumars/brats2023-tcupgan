...
def run_inference(data_dir: str, parameters_file: str, ckpt_file: str, output_dir: str) -> None:
    ...
    list_of_case_ids = ['-'.join(os.path.basename(folder).split('-')[2:4])
                        for folder in glob.glob('%s/*' % (data_dir))]
    print(
        f'There are {len(list_of_case_ids)} case folders in the {data_dir} folder')
    ...
    resize_fn = Resize(
        (240, 240), interpolation=transforms.InterpolationMode.NEAREST)
    affine_matrix = np.array([[-1., -0., -0., 0.], [-0., -1., -0., 239.], [
                             0., 0., 1., 0.], [0., 0., 0., 1.]])
    ...
    for each_sample in tqdm.tqdm(list_of_case_ids):
        ...
        with torch.no_grad():
            predicted_mask = torch.squeeze(generator(torch.unsqueeze(
                torch.Tensor(mri_cube).to(device), 0)), 0).cpu().numpy()

        background = 1 - \
            torch.sum(torch.Tensor(predicted_mask), axis=1, keepdim=True)
        combined_pred = np.concatenate(
            [background.cpu().numpy(), predicted_mask], axis=1)
        normed_combined_pred = torch.Tensor(
            combined_pred) / torch.sum(torch.Tensor(combined_pred), axis=(1), keepdim=True)

        combined_pred_mask = rearrange(resize_fn(torch.argmax(torch.Tensor(
            normed_combined_pred), dim=1)), 'z h w -> h w z').cpu().numpy()

        processed_mask = process_mask_tissue_wise(
            combined_pred_mask, mean_lesion_span_thresh=lesion_threshs, lesion_span_len_thresh=lesion_len_thresh)
